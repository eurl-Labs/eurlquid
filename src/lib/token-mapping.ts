...
export const getTokenInfo = (address: string): TokenInfo | null => {
  ...
  // Debug: log unknown addresses
  console.log('Unknown token address:', address);
  return null;
};

// Helper function to get DEX info by name
export const getDexInfo = (name: string): DexInfo | null => {
  return DEX_MAPPING[name] || null;
};

// Helper function to format token amount (basic implementation)
