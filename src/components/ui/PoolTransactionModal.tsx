        ...
        >
          ...
          <div className="px-5 py-4 bg-[#101828]">
            <div className="flex items-center justify-between mb-3">
              {Array.from({ length: progressConfig.totalSteps }, (_, index) => {
                    ...
                    {stepNum < progressConfig.totalSteps && (
                      <div 
                        className={`w-8 h-1 mx-2 rounded-full transition-all duration-300 ${
                          completedSteps.has(stepNum) ? "bg-[#00FF85]" : "bg-[#2D3748]"
                        }`}
                      />
                    )}
                  </div>
          ...
          <div className="p-5">
              ...
              <div className="mb-6">
                <div className="flex items-center justify-center space-x-4">
                  <div className="text-center">
                    ...
                    <p className="text-sm font-medium text-white">{tokenA.symbol}</p>
                  </div>
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-sm font-medium text-white">{tokenB.symbol}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {step === "success" && (
              <div className="text-center mb-6">
                ...
                <p className="text-lg font-semibold text-white mb-2">
                  {mode === "add-liquidity" ? "Liquidity Added Successfully!" : "Pool Created Successfully!"}
                </p>
                ...
                {txHash && (
                  <div className="mt-4">
                    <p className="text-sm text-[#B0B7C3] mb-2">Transaction Hash:</p>
                    <div 
                    ...
                    <p className="text-xs text-[#B0B7C3] mt-2">
                      Click the link icon to view your transaction on Sonic Explorer
                    </p>
                  </div>
                )}
                ...
                <div className="flex items-center space-x-3">
                  ...
                  {(step === "create-pool" || step === "add-liquidity") && (
                    <>
                      ...
                      <span className="text-sm text-[#B0B7C3]">
                        {step === "create-pool" ? "Creating pool..." : "Adding liquidity..."}
                      </span>
                    </>
                  )}
                </div>
                  ...
                  <p className="text-sm text-red-400 break-words leading-relaxed">
                    {(() => {
                      ...
                      if (cleanError.includes('Contract Call:')) {
                        cleanError = cleanError.split('Contract Call:')[0].trim();
                      }
                      
                      // Remove "Raw Call Arguments:" part and everything after it
                      if (cleanError.includes('Raw Call Arguments:')) {
                        cleanError = cleanError.split('Raw Call Arguments:')[0].trim();
                      }
                      
                      // Remove "Request Arguments:" part and keep only the first part
                      if (cleanError.includes('Request Arguments:')) {
                        const parts = cleanError.split('Request Arguments:');
                        cleanError = parts[0].trim();
                        
                        // If the first part is too short, add a bit from the second part
                        if (cleanError.length < 50 && parts[1]) {
                          const secondPart = parts[1].trim();
                          const fromTo = secondPart.match(/from: (\w+) to: (\w+)/);
                          if (fromTo) {
                            cleanError += ` (${fromTo[1].slice(0,6)}...${fromTo[1].slice(-4)} to ${fromTo[2].slice(0,6)}...${fromTo[2].slice(-4)})`;
                          }
                        }
                      }
                      
                      // Remove version info
                      cleanError = cleanError.replace(/Version: viem@[\d.]+/g, '').trim();
                      
                      // Remove docs links
                      cleanError = cleanError.replace(/Docs: https:\/\/[^\s]+/g, '').trim();
                      ...
                      if (!cleanError || cleanError.length < 10) {
                        cleanError = 'User rejected the transaction request.';
                      }
            ...
            <button
              ...
              className={`w-full py-3 px-4 rounded-lg font-bold text-sm transition-all duration-200 flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed ${
                error ? "bg-red-600 hover:bg-red-700" : step === "success" ? "bg-[#4ADE80] hover:bg-[#3BCC6D]" : "bg-[#1E293B]"
              }`}
              style={{
                color: "#FFFFFF",
              }}
            >
              {isLoading && !error && step !== "success" && <Loader2 className="w-4 h-4 animate-spin " />}
              <span className={error || step === "success" ? "font-bold" : "font-medium"}>
